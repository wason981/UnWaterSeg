...
class EvalCallback:
    ...
    def on_epoch_end(self, epoch, model_eval,dataset):
        # if epoch % self.period == 0 and self.eval_flag:
        while True:
            self.net = model_eval
            if dataset=='SUIM':
                img_dir=os.path.join(self.dataset_path, "SUIM2022/JPEGImages/")
                gt_dir = os.path.join(self.dataset_path, "SUIM2022/SegmentationClass/")
            elif dataset=='UIIS':
                img_dir=os.path.join(self.dataset_path, "val")
                gt_dir = os.path.join(self.dataset_path, "annotations","val")
            elif dataset=='TrashCan':
                img_dir=os.path.join(self.dataset_path, "val")
                gt_dir = os.path.join(self.dataset_path, "annotations","val")

            pred_dir = os.path.join(self.miou_out_path, "detection-results",dataset)
            ...
            for image_id in tqdm(self.image_ids):
                ...
                #   从文件中读取图像
                # -------------------------------#
                if 'jpg' in image_id.split('.') or 'png' in image_id.split('.'):
                ...
                if 'jpg' in image_id.split('.') or 'png' in image_id.split('.'):
                    image.save(os.path.join(pred_dir, image_id))
                else:
